...
#eddy
eddy_cuda10.2 --imain=eddy_in.nii --mask=eddy_mask.nii --acqp=eddy_config.txt --index=eddy_indices.txt --bvecs=bvecs --bvals=bvals --topup=field --data_is_shelled --slm=linear --niter=8 --fwhm=10,6,4,2,0,0,0,0 --repol --ol_type=both --mporder=8 --s2v_niter=8 --estimate_move_by_susceptibility --cnr_maps --verbose --json=dwi.json --out=dwi_post_eddy
...
mv $intermediate.mif $dwidir/$subj/working.mif
